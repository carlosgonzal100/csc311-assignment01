...
public class Truck extends Vehicle {
    ...
    public double rangeInMiles() {
        double loadFactor = 1.0 - Math.min(0.35, getPayloadKg() / 20000.0);
        return getFuelCapacity() * getFuelType().getMilesPerUnit() * loadFactor;
    }

    @Override
    public String toString() {
        return String.format("%s -> %s, payload=%.1f kg, range=%.1f mi", category(), super.toString(), getPayloadKg(), rangeInMiles());
    }
